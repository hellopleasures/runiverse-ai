...
export function RoninExtensionConnectButton() {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      // 3) Otherwise, if send is available, use it (old .send):
      ...
      // 4) Otherwise, if sendAsync is available, use it:
      else if (typeof window.ronin.sendAsync === 'function') {
        accounts = await new Promise((resolve, reject) => {
          window.ronin?.sendAsync(
            { method: 'eth_requestAccounts', params: [] },
            (err: any, response?: { result?: string[] }) => {
              if (err) {
                return reject(err);
              ...
              if (response && Array.isArray(response.result)) {
                ...
                reject(new Error('No accounts received from ronin.sendAsync.'));
              }
            }
          );
        });
